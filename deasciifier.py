...
class Deasciifier(object):
  ...
  def turkish_toggle_accent(self, pos):    
    alt = None
    if self.text[pos] in TurkishTables.turkish_toggle_accent_table:
  ...
  def turkish_need_correction(self, pos):
    ...
    ch = self.text[pos]    
    if ch in TurkishTables.turkish_asciify_table: 
      tr = TurkishTables.turkish_asciify_table[ch]    
    else:
      tr = ch
    
    m = False
    if tr.lower() in TurkishTables.turkish_pattern_table:
  ...
  def turkish_get_context(self, pos, size):
    ...
    while i<len(s) and current_char<text_len-1 and self.text[current_char]!=u' ':
      ...
      space = (c==u' ')
      x = None
      if c in TurkishTables.turkish_downcase_asciify_table:
    ...
    while i>=0 and current_char>0: # TODO: add (and not end of buffer)
      ...
      x = None
      if c in TurkishTables.turkish_upcase_accents_table: # get the uppercase version of the character
